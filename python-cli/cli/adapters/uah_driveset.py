"""
Adapter: UAH-DriveSet -> Telemachus format
...
Output: Telemachus Telemachus Parquet (multi-rate 10Hz IMU / 1Hz GPS)
"""
...
def convert_trip(trip_dir: Path, output_dir: Path, freq_hz: int = 10) -> Path:
    """Convert a single UAH-DriveSet trip folder to Telemachus Telemachus Parquet.
...
def adapt(input_dir: str, output_dir: str) -> list[Path]:
    """Convert all UAH-DriveSet trips in input_dir to Telemachus format.
